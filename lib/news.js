...
function parseRSS(xml, source, defaultCategory) {
  ...
  for (const match of blocks) {
    ...
    // Skip articles where title looks like a URL or is empty
    if(!title || title.startsWith('http') || title.startsWith('www') || title.length < 10) continue;

    const link    = get('link') || (block.match(/<link>([\s\S]*?)<\/link>/)?.[1]?.trim()) || '';
    const pubDate = get('pubDate') || get('dc:date') || get('published') || '';
    const rawDesc = get('description');
    // Google News RSS puts HTML in description — strip all tags aggressively
    const desc = rawDesc
      .replace(/<!\[CDATA\[[\s\S]*?\]\]>/g, '') // remove any nested CDATA
      .replace(/<a[^>]*href=["'][^"']*["'][^>]*>[\s\S]*?<\/a>/gi, '') // remove anchor tags + content
